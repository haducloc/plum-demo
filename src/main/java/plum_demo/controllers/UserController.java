...
public class UserController {
	...
	if (model.getUserId() == null && request.getModelState().isValid("username")) {
		...
		request.getModelState().addError("username", request.res("user_edit.username_already_exists", model.getUsername()));
	    }
	}
	...
	try {
		...
		if (user.getUserId() == null) {
		    user.setPassword(PasswordDigester.DEFAULT.digest(user.getPassword()));
		}
